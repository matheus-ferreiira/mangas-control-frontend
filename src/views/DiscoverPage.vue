                        ...
                        >
                            ...
                            <div style="position: absolute; bottom: 18px; left: 18px; right: 18px;">
                                ...
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <button
                                        style="padding: 10px 20px; border-radius: 12px; border: none; background: #f5a623; color: #050608; font-size: 12px; font-weight: 800; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; box-shadow: 0 0 24px hsl(38 91% 55% / 0.45);"
                                    ...
                                    <div v-if="homeData.featured.is_in_library" style="width: 36px; height: 36px; border-radius: 18px; background: rgba(245,166,35,0.18); border: 1.5px solid rgba(245,166,35,0.4); display: flex; align-items: center; justify-content: center;">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f5a623" stroke-width="2.5" stroke-linecap="round"><path d="m4 12 5 5 11-11"/></svg>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div style="position: relative; border-radius: 12px; overflow: hidden; height: 116px; background: rgba(255,255,255,0.025);">
                                            ...
                                            <div style="position: absolute; bottom: 0; left: 0; right: 0; padding: 8px 10px;">
                                                ...
                                                <div style="height: 3px; background: rgba(255,255,255,0.1); border-radius: 3px; overflow: hidden;">
                                                    <div style="height: 100%; border-radius: 3px; transition: width 0.3s ease; background: #f5a623;" :style="{ width: ci.progress_pct + '%' }"></div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <section v-for="section in homeSections" :key="section.id" style="margin-bottom: 28px; animation: fade 0.3s ease;">
                            <div style="display: flex; align-items: flex-end; justify-content: space-between; padding: 0 18px; margin-bottom: 12px;">
                                ...
                                <button
                                    ...
                                    style="display: flex; align-items: center; gap: 3px; background: transparent; border: none; cursor: pointer; font-size: 11px; font-weight: 700; color: #f5a623; letter-spacing: 0.04em;"
...
const TYPE_META: Record<string, { color: string; label: string; short: string }> = {
    manga:  { color: '#f5a623', label: 'MANGA', short: 'M' },
...
const CAT_STATUS: Record<string, { label: string; color: string }> = {
    ongoing:   { label: 'Em andamento', color: '#f5a623' },
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            typeCategories: [
                ...
                { type: 'manga' as ContentType,  label: 'Mangá',  icon: '📖', color: '#f5a623' },
    ...
    computed: {
        ...
        homeSections(): HomeSection[] {
            ...
            if (d.popular_manga?.length) {
                sections.push({ id: 'popular_manga', title: 'Mangás mais populares', sub: 'Maior audiência no catálogo', accentColor: '#f5a623', filterType: 'manga', items: d.popular_manga });
            }
            ...
            if (d.top_rated?.length) {
                sections.push({ id: 'top_rated', title: 'Melhor avaliados', sub: 'Score acima de 7.5', accentColor: '#f5a623', items: d.top_rated });
            }
            ...
            if (d.recently_updated?.length) {
                sections.push({ id: 'recently_updated', title: 'Atualizados recentemente', sub: 'Novos capítulos disponíveis', accentColor: '#f5a623', items: d.recently_updated });
            }
    ...
    methods: {
        ...
        typeChipStyle(val: ContentType | null): Record<string, string> {
            ...
            const col = val ? (TYPE_META[val]?.color ?? '#f5a623') : '#e9edf2';
            return {
...
.search-input:focus { border-color: #f5a623 !important; box-shadow: 0 0 0 3px hsl(38 91% 55% / 0.12) !important; }

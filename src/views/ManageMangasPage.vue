            ...
            <div class="px-5 pt-5 pb-24">
                ...
                <div class="bg-neon-surface border border-neon-border rounded-2xl p-5 mb-7">
                    <h3 class="text-[17px] font-bold text-neon-text m-0 mb-4">
                        {{ editingId ? 'Editar' : 'Adicionar ao Acervo' }}
                    </h3>
                    ...
                    <div class="mb-4">
                        ...
                        <div
                            ...
                            :class="(coverPreview || form.cover) ? 'h-[200px] border-neon-accent/20' : 'h-[160px]'"
                        ...
                        >
                            ...
                            <div v-else class="flex flex-col items-center gap-3 p-4 text-center">
                                <div class="w-14 h-14 rounded-2xl bg-neon-surface border border-neon-border flex items-center justify-center">
                                    <IonIcon :icon="imageOutline" class="text-[28px] text-neon-muted" />
                                </div>
                                <div>
                    ...
                    <div class="flex flex-col gap-2.5 mt-1">
                        <IonButton expand="block" class="btn-primary" :disabled="saving || !form.name.trim()" @click="editingId ? updateContent() : createContent()">
                            ...
                            <span v-else>{{ editingId ? 'Atualizar' : 'Adicionar' }}</span>
                        </IonButton>
                ...
                <div v-else-if="filteredContents.length === 0" class="text-center py-8">
                    <IonIcon :icon="bookOutline" class="text-[40px] text-neon-muted block mb-2.5" />
                    <p class="text-sm text-neon-muted m-0">Nenhum item no acervo ainda.</p>
                </div>

                <div v-else>
                    <div v-for="content in filteredContents" :key="content.id" class="flex items-center gap-3 bg-neon-surface border border-neon-border rounded-[14px] p-3 mb-2.5">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col gap-1">
                            <div class="flex items-center gap-1.5">
                                <span class="text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded" :class="getTypeColor(content.type)">
                        ...
                        <div class="flex gap-2 flex-shrink-0">
                            <button class="w-[34px] h-[34px] rounded-[10px] bg-neon-elevated border border-neon-border flex items-center justify-center text-base text-[#7b8ff5] cursor-pointer" @click="startEdit(content)">
...
import { bookOutline, tvOutline, libraryOutline, pencilOutline, trashOutline, imageOutline } from 'ionicons/icons';
import {
...
export default defineComponent({
    ...
    components: { IonPage, IonHeader, IonToolbar, IonTitle, IonContent, IonButtons, IonBackButton, IonButton, IonIcon, IonSpinner, IonInput },
    data() {
        return {
            loading: false,
            saving: false,
...
.btn-cancel { --background: #1a2035; --color: #e8eaf0; --border-radius: 12px; height: 44px; }

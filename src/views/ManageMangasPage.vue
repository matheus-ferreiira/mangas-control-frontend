  ...
  <ion-page>
    <ion-header>
      <ion-toolbar>
        ...
        <ion-title>Manga Catalog</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content :fullscreen="true">
      <div class="content-pad">
        <!-- Add / Edit form -->
        <div class="form-card">
          <h3 class="form-title">{{ editingId ? 'Edit Manga' : 'Add to Catalog' }}</h3>

          <div class="field-group">
            <span class="neon-label">Name</span>
            <div class="input-row">
              <input v-model="form.name" placeholder="Series title..." class="field-input" />
            </div>
          </div>

          <div class="field-group">
            <span class="neon-label">Cover URL</span>
            <div class="input-row">
          ...
          <div class="field-group">
            <span class="neon-label">Total Chapters</span>
            <div class="input-row">
              <input
          ...
          <div class="btn-row">
            ...
            >
              ...
              <span v-else>{{ editingId ? 'Update' : 'Add Manga' }}</span>
            </ion-button>
            <ion-button
              v-if="editingId"
              class="cancel-btn"
        ...
        <div v-else>
          <div v-for="manga in mangas" :key="manga.id" class="manga-row">
            ...
            <div class="manga-info">
              ...
              <span class="manga-ch" v-if="manga.total_chapters">{{ manga.total_chapters }} chapters</span>
            </div>
  ...
  methods: {
    ...
    async createManga() {
      ...
      try {
        ...
        this.resetForm()
        await this.showToast('Manga added to catalog!', 'success')
      } catch {
        await this.showToast('Failed to add manga.', 'danger')
      } finally {
        this.saving = false
      }
    },
    ...
    async updateManga() {
      ...
      try {
        ...
        this.cancelEdit()
        await this.showToast('Manga updated!', 'success')
      } catch {
        await this.showToast('Failed to update manga.', 'danger')
      } finally {
        this.saving = false
      }
    },
    ...
    async confirmDelete(manga: Manga) {
      const alert = await alertController.create({
        header: 'Delete Manga',
        message: `Delete "${manga.name}" from the catalog?`,
        buttons: [
          { text: 'Cancel', role: 'cancel' },
          {
            text: 'Delete',
            role: 'destructive',
            handler: () => this.deleteManga(manga.id),
          },
        ],
      })
      await alert.present()
    ...
    async deleteManga(id: number) {
      try {
        ...
        await this.showToast('Manga deleted.', 'success')
      } catch {
        await this.showToast('Failed to delete manga.', 'danger')
      }
    },
...
.save-btn {
  ...
  --border-radius: 12px; font-weight: 700; height: 48px;
}
